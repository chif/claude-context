 ...
 */
    private async processFileList(
        ...
        const CHUNK_LIMIT = parseInt(envManager.get('CHUNK_LIMIT') || '0', 10); // 0 = no limit
        console.log(`[Context] 🔧 Using EMBEDDING_BATCH_SIZE: ${EMBEDDING_BATCH_SIZE}`);
        console.log(`[Context] 🔧 Using CHUNK_LIMIT: ${CHUNK_LIMIT === 0 ? 'unlimited' : CHUNK_LIMIT}`);
        ...
        for (let i = 0; i < filePaths.length; i++) {
            ...
            try {
                ...
                for (const chunk of chunks) {
                    ...
                    if (CHUNK_LIMIT > 0 && totalChunks >= CHUNK_LIMIT) {
